...
@IonicPage()
@Component({
  selector: 'page-comic-favourite',
  templateUrl: 'comic-favourite.html',
})
export class ComicFavouritePage {
	FBData: any;
	...
	public myFav: Array<any> = [];
  	public favoRef: firebase.database.Reference = firebase.database().ref(`/Favourite-Comic`);
	    ...
	    this.gPlus.trySilentLogin({}).then(stats => {
	      if(stats){
	        ...
	        this.favoRef.orderByChild("userID").equalTo(this.userIDs).on('value', dataFavourite => {
	          ...
	          if(this.myFav){
	            console.log('=> Favourite Found. Count the Data :', this.myFav.length);
	            console.log('=> The Data is :', this.myFav);
	            this.isHaveFavourite = true;
	          }
	          else{
